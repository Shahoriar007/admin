...
var parseUrl = function parseUrl() {
  ...
  for (var i = 0; i < params.length; i++) {
    ...
    if (!!items[_key]) {
      ...
      localStorage.setItem(items[_key].localStorage, value); // Update local variables
...
var updateBodyClasses = function updateBodyClasses() {
  ...
  document.body.classList.add("theme-".concat(config.theme)); // for (const [key, params] of Object.entries(items)) {
...
var submitForm = function submitForm(form) {
  ...
  for (var _i3 = 0, _Object$entries3 = Object.entries(items); _i3 < _Object$entries3.length; _i3++) {
    ...
    localStorage.setItem(_params2.localStorage, value); // Update local variables
...
parseUrl(); // Update body classes

updateBodyClasses(); // Elements

var form = document.querySelector('#offcanvasSettings'); // Toggle form controls
